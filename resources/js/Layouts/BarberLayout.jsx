...
import { IconCalendarEvent, IconHome, IconPlus, IconUserCircle } from '@tabler/icons-react';
...
export default function BarberLayout({
    ...
    const { pendingTurnosCount } = usePage().props;
    const inicioActive = route().current('barber.dashboard');
    const turnosActive = route().current('barber.turnos.*');
    ...
    return (
        <div className="panel-theme min-h-screen bg-brand-bg">
            ...
            <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-brand-border bg-brand-nav-bg pb-[env(safe-area-inset-bottom)]">
                <div className="relative mx-auto flex h-16 max-w-3xl items-center justify-between px-6">
                    <div className="flex items-center gap-6">
                        <Link
                            href={route('barber.dashboard')}
                            aria-current={inicioActive ? 'page' : undefined}
                            data-tour="barber-nav-inicio"
                            className={
                        ...
                        <Link
                            href={route('barber.turnos.index')}
                            aria-current={turnosActive ? 'page' : undefined}
                            data-tour="barber-nav-turnos"
                            className={
                                'relative flex min-h-[44px] min-w-[44px] flex-col items-center justify-center gap-0.5 rounded-brand-sm text-xs font-medium transition ' +
                                (turnosActive ? 'text-brand-nav-active' : 'text-brand-nav-text hover:text-brand-nav-active')
                            }
                        >
                            <IconCalendarEvent size={24} stroke={turnosActive ? 2 : 1.75} />
                            Turnos
                            {pendingTurnosCount > 0 && (
                                <span className="absolute right-0 top-0 inline-flex h-4 min-w-[16px] items-center justify-center rounded-full bg-brand-warning px-1 text-[10px] font-bold text-brand-on-primary">
                                    {pendingTurnosCount}
                                </span>
                            )}
                        </Link>
