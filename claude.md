...
- **Única excepción al Global Scope vía `withoutGlobalScope()`**: `App\Http\Controllers\Admin\OwnerController` (vista de soporte de solo lectura, Fase 9) es el ÚNICO lugar del proyecto donde está permitido bypasear `BelongsToBarberiaScope` explícitamente (vía `withoutGlobalScope`), porque el admin necesita ver datos across todos los tenants por diseño. Cada bypass debe ir documentado con un comentario explícito en la query. Ningún otro controller autenticado (Owner, Barber, ni futuros) debe tomar esto como precedente — si otro lugar del código necesita bypasear el scope, es señal de un bug de tenancy, no de un caso legítimo nuevo.
- **Segundo contexto documentado sin scope: reserva pública de turnos (sin auth)**: `App\Http\Controllers\Public\PublicTurnoController` (rutas `/turno/{barberia:public_slug}`, sin middleware `auth`) es el único lugar del proyecto que corre sin usuario autenticado sobre modelos con `BelongsToBarberiaScope` (Servicio, Turno). No es un bypass vía `withoutGlobalScope()`: sin `Auth::user()`, el scope ya cae en la misma rama de early-return que usa para admin y no filtra nada (ver comentario en `BelongsToBarberiaScope`). La seguridad no depende de eso — cada query del controller filtra explícitamente por `barberia_id`, resuelta por `public_slug` vía route model binding (nunca por input del cliente), igual que la capa extra que ya aplica `Owner\TurnoController`. `DisponibilidadService` no necesitó ningún cambio: sus queries internas ya filtraban explícitamente por `barberia_id`/`user_id` sin depender del scope.
...
- **`features` vs. `included_items` (no confundir)**: `features` es un json de flags booleanos (catálogo en `Plan::KNOWN_FEATURES`) que, cuando tiene entradas, gatea funcionalidad real en código — hoy `KNOWN_FEATURES` está vacío: ningún feature real está restringido por plan, todo el catálogo escala únicamente por cantidad de barberías y barberos (`max_barberias`/`max_barberos`). El mecanismo (`hasFeature()`, la columna `features`, el catálogo `KNOWN_FEATURES` con su checkbox dinámico en Admin → Planes) sigue existiendo tal cual, disponible para gatear una función puntual el día que el negocio lo defina explícitamente — no se generaliza a otras vistas sin esa decisión. `included_items` es un array de strings puramente descriptivo (lo que el owner ve al elegir el plan en el registro, y lo que el equipo ve como referencia en Admin → Planes): no activa nada por sí solo. Si algún día un ítem de `included_items` menciona algo que en realidad depende de un feature flag, quien edite el catálogo debe activar también ese flag — si no, el plan promete algo que no cumple. Cada plan carga su propia lista completa y autocontenida (nunca "todo lo anterior +", eso es solo para lectura humana en la especificación de negocio).
...
- **Encuestas — el premio (cupón automático) aplica exclusivamente a owners**: los barberos pueden responder encuestas (`Survey` con `target_audience=barber/both`) igual que los owners, pero nunca reciben cupón, ya que no tienen suscripción propia que descontar. `Survey::grantsRewardTo()` es el único punto que decide esto (`reward_type=coupon && $user->isOwner()`) y lo verifica `SurveyRewardService::grant()` antes de generar nada — si una encuesta con `reward_type=coupon` es respondida por un barbero (posible cuando `target_audience=both`), se guarda la respuesta normalmente y el método corta ahí, sin intento de cupón. Cualquier fallo generando o aplicando el cupón (colisión de code tras reintentos, cupón no redimible, etc.) se loguea en `system_error_logs` y nunca revierte ni bloquea el guardado de `survey_responses`/`survey_answers`, que ya quedaron persistidos en una transacción previa e independiente.
...
- **2026-07-19 — Acciones de barbero centralizadas en la card del listado, no en el perfil**: `resources/js/Pages/Owner/Barberos/Index.jsx` (`Owner/Barberos/Show.jsx` es de solo lectura para seguimiento de rendimiento) es donde viven hoy "Editar" y "Dar de baja" para cada barbero, dentro de su card en el listado. Si en el planteo original de esa pantalla se pensó centralizarlas en el perfil, ese no es el comportamiento vigente — cualquier trabajo nuevo sobre gestión de barberos (incluidos tours u otras guías de UI) debe apuntar a las acciones del listado.
- **2026-08-02 — `BelongsToBarberiaScope` sin usuario autenticado: rama sin ejercitar hasta la reserva pública de turnos**: hasta construir `PublicTurnoController`, todo lo que corría contra modelos con el scope lo hacía autenticado (owner, barber, o admin con su bypass documentado) — nadie había ejercitado la rama `! $user` de `BelongsToBarberiaScope::apply()`. Se confirmó leyendo el código (no hizo falta cambiar nada) que esa rama ya devuelve sin filtrar, el mismo camino que toma para admin — no rompe ni devuelve vacío. Como la seguridad de `PublicTurnoController` no depende de eso (cada query filtra explícitamente por `barberia_id` resuelta por `public_slug`), no hizo falta ningún `withoutGlobalScope()` nuevo, solo documentar la rama en el docblock de la clase. Ver regla en "Segundo contexto documentado sin scope" arriba.